...
def setup_options(make_option):
    ...
    make_option("--migrations", action="append", type="string", dest="migrations",
...
def _list_dbs(*args):
    print("Available --migrations options (use --dburi to override)")
    for macro in sorted(file_config.options('migrations')):
        print("%20s\t%s" % (macro, file_config.get('migrations', macro)))
    sys.exit(0)
    ...
    if options.db:
        for db_token in options.db:
            for db in re.split(r'[,\s]+', db_token):
                if db not in file_config.options('migrations'):
                    raise RuntimeError(
                        "Unknown URI specifier '%s'.  "
                        "Specify --dbs for known uris."
                        % db)
                else:
                    db_urls.append(file_config.get('migrations', db))

    if not db_urls:
        db_urls.append(file_config.get('migrations', 'default'))
...
def stop_test_class(cls):
    ...
    #assert not inspect(testing.migrations).get_table_names()
