...
class ReferencePropertyField(fields.SelectFieldBase):
    """
    A field for ``migrations.ReferenceProperty``. The list items are rendered in a
    select.

    :param reference_class:
        A migrations.Model class which will be used to generate the default query
...
class StringListPropertyField(fields.TextAreaField):
    """
    A field for ``migrations.StringListProperty``. The list items are rendered in a
    textarea.
    """
    def _value(self):
        if self.raw_data:
